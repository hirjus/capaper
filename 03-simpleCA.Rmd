...
@RefWorks:doc:5a857a43e4b0ed2d44664d75 korostavat ranskalaisen perinteen mukaisesti
...
Esitän tässä luvussa korrespondenssianalyysin peruskäsitteet intuitiivisesti,
...
numeerisista tuloksista. Analysoitava taulukko, sen rivien ja sarakkeiden
riippuvuuksia kuvaava kartta ja kartan perustana olevat numeeriset tulokset
esitetään yhdessä. Näin tulkinnan perussäännöt on helpompi ymmärtää.
...
Tässä tutkielmassa käytetystä aineistosta on poistettu havainnot, joissa tieto vastauksesta puuttuu.
...
Testisuure saadaan laskemalla yhteen jokaisen solun havaittujen ja odotettujen
...
inertia). Se kuvaa, kuinka paljon varianssia taulukossa on ja inertia on riippumaton
...
riviprofiilien keskiarvoprofiilin vastaavat alkiot $r_{i}$ .
...
Inertia voidaan esittää rivien ja keskiarvorivin (sentroidin) $\chi^{2}$ -etäisyyksien
...
pätee riveille. Tämä rivi- ja sarakeongelmien duaalisuus on yksinkertaisen
...
khii2-etäisyys (käytän tekstissä tätä kirjoitusasua) ja tässä massat ovat mukana
...
hajonnasta on kaksiulotteisessa projektiossa saatu kuvattua akseleille.
...
ei aloiteta pisteiden etäisyyksiä vertailemalla.
...
Ensimmäisen dimension tulkinta on aika selkeä. Toinen akseli on kontrasti
...
En jatkossa esitä kuvailevia akseleiden nimiä kuvissa, akseleiden tulkinta olisi
kumminkin analyysin ensimmäinen tehtävä.

Maiden vertailu tehdään näiden akselien suuntaan. Sekä sarakepisteiden että
...
ja transformaatiot. Tämä oli menetelmän perusidea jo Benzecrillä (CAiP, s. 201).
...
Symmetrisessä kartassa (Kuva \@ref(fig:simpleCA1map1)) molemmat pisteparvet on esitetty
pääkoordinaateissa (principal coordinates) ikään kuin päällekkäin, samassa kuvassa.
...
kartassa (Kuva \@ref(fig:G1-3asymm2)) sarakkeet on esitetty standardikoordinaateissa
ja rivit pääkoordinaateissa.

Sarakepisteitä kutsutaan ideaalipisteiksi, ne edustavat kuvitteellisia maita, joissa
...
Kartassa (Kuva \@ref(fig:simpleCA1map1)) on esitetty myös pisteiden massat. Tässä aineistossa
pisteiden tai sarakesymbolien kokoerot eivät kovin selvästi erotu, mutta yleiskuvan
ne kertovat.

Rivipisteiden ja sarakkeiden _kontribuutiot_ voidaan esittää kartalla
värisävyinä.
...
Kaikki edellä esitetyt päättelyt perustuvat tietysti kaksiulotteiseen projektioon.
...
Sarake- ja rivipisteet esitetään siinä ikään kuin päällekkäin, samassa koordinaatistossa.
...
pääakselien suunnassa vastaavilla pääakselien inertioiden neliöjuurilla.

Kun akselien inertioiden (principal inertias) neliöjuuret eivät ole liian erisuuruisia,
ideaalipisteiden suuntavektorit ovat lähes saman suuntaisia pääkoordinaateissa ja
standardikoordinaateissa.

Jos pääinertioiden neliöjuuret ovat hyvin eri suuruisia, tulkintaongelmia voi tulla, mutta
niillä ei käytännössä ole merkitystä. Siksi Greenacre pitää skaalausdebattia akateemisena
...
Symmetrinen kartta on hyvä vaihtoehto, sillä asymmetrisessä kartassa skaalaus vie
...
ja sarakkeiden suuntavektoreille projisoiduilla pistellä on kaksoiskuvatulkinta.
...
yhteyden ratkaisualgoritmiin, singulaariarvohajotelmaan.
