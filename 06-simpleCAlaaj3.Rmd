...
(Greenacre ja Pardo artikkelikokoelmassa [@RefWorks:doc:5ab76b43e4b003f4468d1f07], s. 198).
...
[@RefWorks:doc:5a857a44e4b0ed2d44664d87]. Artikkelin laajennetussa versiossa (emt.)
esimerkkiaineistona on ISSP:n 1994 data. Selkeä oppikirjaesitys on CAiP (ss. 161-). 


Eräs sovelluskohde yhteiskuntatieteellisissä kyselyaineistoissa on
...
Kartan \@ref(fig:maagaCA1map1) avulla  aineiston voi jakaa kahteen ryhmään.
Suomi, Tanska ja Saksa ovat pääakselin oikealla puolella. Bulgaria ja Unkari
ja Belgian kanssa ovat toinen ryhmä.
...
_näyttävät_ erilaisilta. Pääakselien inertioiden neliöjuuret eivät poikkea toisistaan 
huomattavasti( `r round(sqrt(0.10709),3)` ja `r round(sqrt(0.02498),3)` ), joten 
...
samoin muutama muu huonosti tasoon sijoitettu piste.Huonosti kuvatuista pisteistäkuva
...
Kartasta \@ref(fig:maagaCA2sub2map2) nähdään, että naisten ikäluokat ovat
kaikissa maissa liberaalimpia kuin vastaavat miesten ikäluokat. Nuorin ikäluokka
on konservatiisemmalla puolella ja samalla toisella dimensiolla maltillisemmalla
puolella. Vanhemmat ikäluokat ovat konservatiivisempia.
...
Belgian, Unkarin ja Bulgarian kartalla ensimmäisen dimension dimension tulkinta
...
( `r round(0.143551,3)` + `r round(0.119602,3)` =  `r round((0.143551 + 0.119602),3)`).
Selitysasteet nousevat hieman, ja aineiston riippuvuuden rakenteesta saadaaan esiin
selviä eroja. Osajoukkojen analyysi täydentää ja tarkentaa
yleiskuvaa (\@ref:fig:maagaCA1map1).
...
Poikkeavat havainnot eivät ole ongelma, vaan datan ominaisuus.
