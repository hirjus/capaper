...
\date{Versio 1.0, tulostettu 2020-11-26}
...
\hypertarget{tutkielman-tavoite-ja-toteutus}{%
\section{Tutkielman tavoite ja toteutus}\label{tutkielman-tavoite-ja-toteutus}}
...
analyysin ''\ldots perussäännöt tulkinnalle. Kaikki muut korrespondenssianalyysin
...
interpretation)''(\citet{RefWorks:doc:5a857a44e4b0ed2d44664d84} s. 437).
...
järjesti ensimmäisen konferenssin 1991 ( \url{http://www.carme-n.org} ).

Korrespondenssianalyysi esitellään oppikirjoissa muiden menetelmien rinnalla.
MG (ecological data)\citep{RefWorks:doc:5b8175eee4b08da192f99a35}, kaksoisikuvat ja
compositional data \citep{RefWorks:doc:5f934955e4b08d20366a5bd1}. Suomessa
korrespondenssianalyysi esitellään kahdessa oppikirjassa
(\citet{RefWorks:doc:5a857a44e4b0ed2d44664d95} , \citet{RefWorks:doc:5a857a44e4b0ed2d44664da4}).
...
oppikirjojensa nimet eivät turhaa pääty ``\ldots in practice''.
...
\hypertarget{tutkielman-rakenne}{%
\section{Tutkielman rakenne}\label{tutkielman-rakenne}}
...
ca-pakettia( \citet{ca2007} ).

Tutkielman koodi on julkaistu Github-palvelussa (\url{https://github.com/hirjus/capaper}),
samoin tutkielman käsikirjoitus ( \url{https://hirjus.github.io/capaper/} )
...
\hypertarget{tuxe4rkeimmuxe4t-luxe4hteet}{%
\section{Tärkeimmät lähteet}\label{tuxe4rkeimmuxe4t-luxe4hteet}}

Tutkielman tärkein lähde on \citep{RefWorks:doc:5a857a43e4b0ed2d44664d78} (jatkossa CAiP),
ja muitakin saman tekijän artikkeleita on käytetty paljon. Kevään 2017
kurssimateriaali\citep{RefWorks:doc:5b6ef091e4b0984fd9b8c0ca} ja laskuharjoitusten koodi
on ollut hyödyllinen pikaopas peruskäsitteisiin ja r-toteutuksen ratkaisuihin.

\citet{RefWorks:doc:5ab76b43e4b003f4468d1f07} toimittama Carme-verkoston kolmas
...
tehdä heti kun data luetaan. \citet{RefWorks:doc:5c3759c2e4b0085b307c82b5} kertovat,
...
{\centering \includegraphics[width=0.9\linewidth]{img/simpleCAasymmTulk2} 

}

\caption{Rivipisteiden projektiot}\label{fig:G1-3-asymmtulk2}
\end{figure}
...
Tätä ''supertaulukkoa'' kutsutaan \emph{Burtin matriisiksi}.
\emph{Indikaattorimatriisi} on toinen tapa esittää data. Indikaattorimatriisin
...
tiedolle. Analyysi kannattaa aloittaa tästä pulmasta, yksi hyvä ratkaisu on
osajoukon MCA (subset MCA).
...
pienemmäksi (adjusted inetia, kts. liite 1),
...
mukaisesti näiden välisellä U-muotoisella linjalla (Guttman-efekti).

Osajoukon korrespondessianalyysin esitelleissä artikkeleissa
...
\citet{RefWorks:doc:5a857a43e4b0ed2d44664d7c} (s. 139-) analysoi lähes samoja kysymyksiä
ISSP-datalla 2012. Tulokset ovat hyvin samantapaisia. Karttaa \ref(fig:subsetMCA1map3)
...
yksinkertaista kuin ehkä luullaan. \citet{RefWorks:doc:5b15542ee4b0e2616bc42dca}
tutkivat MCA-menetelmällä ISSP:n 1994 kerättyä aineistoa. He osoittavat, että
datan laatu ja rakenne vaihtelee paljon maiden välillä (''\ldots both the quality of
...
jonkinlaisilla keskiarvoilla? \citet{RefWorks:doc:5c223412e4b0508a6674dec0} osoittavat
...
Tutkielma on tehty r-ohjelmistolla ja raportointi rmarkdown - koodilla. Teksti
ja koodi ovat samassa dokumentissa, samoin analyysin tulokset. Ne voi tulostaa
eri formaateissa, ja Github.com - palvelussa voi versionhallinnan lisäksi julkaista
html-raportit. Ongelmiakin oli, ne ja teknisen ympäristön yksityiskohdat on
...
\textbf{edit poistetava pätkä loppuu tähän}

\textbf{Monimuuttujakorrespondenssianalyysi MCA}
...
Bookdown-paketti laajentaa R-ohjelmiston julkaisuominaisuuksia \citep{RefWorks:doc:5b6b36dde4b09b7ec442bf8b}.
...
bookdown-paketin tulostustvaihtoehto html\_book.
...
Tutkielman luonnos (\url{https://hirjus.github.io/capaper/}) ja lähdekoodi
(\url{https://github.com/hirjus/capaper}) löytyvät sieltä. Data on pakattuna tiedostona.
...
TinyTeX - versiota (\url{https://yihui.org/tinytex/}) ja
...
\CommentTok{# eval = FALSE 24.11.2020}
...
\KeywordTok{plot}\NormalTok{(maagaCA2sub2, }\DataTypeTok{main =} \StringTok{"Q1b: Lapsi kärsii jos äiti käy töissä"}\NormalTok{,}
     \DataTypeTok{sub =} \StringTok{"symmetrinen kartta - osajoukko Tanska, Saksa ja Suomi)"}
\NormalTok{   )}
...
\KeywordTok{plot}\NormalTok{(Concat1jh.CA1, }\DataTypeTok{map =} \StringTok{"rowgreen"}\NormalTok{,}
     \CommentTok{#mass = c(TRUE, TRUE) ei oikein erotu kuvassa}
     \DataTypeTok{contrib =}\KeywordTok{c}\NormalTok{(}\StringTok{"relative"}\NormalTok{, }\StringTok{"absolute"}\NormalTok{),}
     \DataTypeTok{arrows =} \KeywordTok{c}\NormalTok{(}\OtherTok{FALSE}\NormalTok{, }\OtherTok{TRUE}\NormalTok{),}
     \DataTypeTok{main =} \StringTok{"Pinottu taulukko, kontribuutiokartta"}\NormalTok{,}
     \DataTypeTok{sub =}  \StringTok{"    sarakkeet absoluuttinen kontribuutio, rivit suhteellinen,}
\StringTok{     taustamuuttujat a1-a6 ikäluokat, m = miehet, f = naiset"}
\NormalTok{     )}

\CommentTok{# Tätä dataa ei käytetä - pelkät kysymykset}
...
\CommentTok{#Pelkät kysymykset}

\CommentTok{#missingMCAvars2 <- c("Q1a","Q1b", "Q1c", "Q1d","Q1e","Q2a","Q2b")}
\CommentTok{#missingTestMCA2.dat <- ISSP2012jh1d.dat %>% select(all_of(missingMCAvars2))}
\CommentTok{#sum(!complete.cases(missingTestMCA2.dat))}
\CommentTok{# puuttuvia tietoja 4554}
\CommentTok{# 4553/32823 = 0.1387137}
...
\KeywordTok{plot.mjca}\NormalTok{(Qmuuttujat1.mca, }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{),}
          \DataTypeTok{main =} \StringTok{"Symmetrinen kartta"}\NormalTok{,}
          \DataTypeTok{sub =} \StringTok{"Kysymykset Q1a, Q1b, Q1c, Q1d, Q1e, Q2a, Q2b, vastaukset S-s-?-e-E-P "}
\NormalTok{          )}
...
\KeywordTok{plot.mjca}\NormalTok{(Qmuuttujat1.mca, }\DataTypeTok{what =} \KeywordTok{c}\NormalTok{(}\StringTok{"all"}\NormalTok{,}\StringTok{"all"}\NormalTok{),}\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{2}\NormalTok{),}
          \DataTypeTok{col =} \KeywordTok{c}\NormalTok{(}\StringTok{"lightblue"}\NormalTok{, }\StringTok{"red"}\NormalTok{),}
           \DataTypeTok{main =} \StringTok{"Symmetrinen kartta"}\NormalTok{,}
          \DataTypeTok{sub =} \StringTok{"Kysymykset Q1a, Q1b, Q1c, Q1d, Q1e, Q2a, Q2b,vastaukset S-s-?-e-E-P, havainnot (n = 32 823)"}
\NormalTok{          )}
...
\CommentTok{# Kontribuutiokartta on tarpeeksi selkeä, muutujien tunnukset erottuvat.}
\CommentTok{# Jätetään tämä kuva pois (25.11.20)}
\CommentTok{#}
\CommentTok{#plot.mjca(Qmuuttujat2.mca,}
\CommentTok{#          main="Seitsemän kysymystä, viisi vastausvaihtoehtoa",}
\CommentTok{#          sub = "subset: ei puuttuvien vastausten kategorioita(*P)")}


\CommentTok{# jätettään symmetrinen kartta pois, asymmetrinen on parempi (25.11.20)}
...
\KeywordTok{plot.mjca}\NormalTok{(Qmuuttujat2.mca, }\DataTypeTok{map =} \StringTok{"rowprincipal"}\NormalTok{,}
          \DataTypeTok{what =} \KeywordTok{c}\NormalTok{(}\StringTok{"all"}\NormalTok{,}\StringTok{"all"}\NormalTok{),}\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{2}\NormalTok{),}
          \DataTypeTok{col =} \KeywordTok{c}\NormalTok{(}\StringTok{"lightblue"}\NormalTok{, }\StringTok{"red"}\NormalTok{),}
          \DataTypeTok{main=}\StringTok{"Asymmetrinen kartta - osajoukko: ei puuttuvia vastauksia"}\NormalTok{,}
          \DataTypeTok{sub =} \StringTok{"Kysymykset Q1a, Q1b, Q1c, Q1d, Q1e, Q2a, Q2b, vastaukset S-s-?-e-E-P"}\NormalTok{)}



\CommentTok{# pois 25.11.2020}
