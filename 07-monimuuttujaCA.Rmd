...
jakaa kahteen erilaiseen tutkimusasetelmaan.  Ensimmäisessä tutkitaan kahden
...
analyysiin ei ole aivan yksikertainen asia. Geometrinen tulkinta ei ole
läheskään niin selkeä kuin yksinkertaisessa korrespondenssianalyysissä. 
Silti MCA toimii käytännössä usein hyvin juuri kyselytutkimusten analyysissä. 
MCA-kartat esittävät luokittelumuuttujien arvot kartalla optimaalisesti. 
Jokainen rivipiste (havainto) sijoittuu kartalle valitsemiensa 
vastausvaihtoehtojen keskiarvopisteeseen. MCA-kartalla nämä luokittelumuuttujien
arvoja vastaavat sarakepisteet maksimoivat rivipisteiden hajonnan
kartalla [@RefWorks:doc:5a857a44e4b0ed2d44664d84 , s. 444-445].
...
nuorimman välillä 1. dimension suuntaan. 

Ikäluokkapisteet ovat koko aineiston keskiarvopisteitä, niiden sijaintia voi
...
En esitä numeerisia tuloksia tarkemmin.

Karttaa \@ref(fig:concat1map1) voi verrata karttaan \@ref(fig:gaCA1map), jossa on esitetty iän ja sukupuolen yhteisvaikutusmuuttuja. Pinottu taulu on vaihtoehtoinen tapa, ja kartasta 
...
Kontribuutionkartan (Kuva 7.3) sarakkeista E-sarake (täysin eri mieltä) määrittää akseleita
...
En tässä jaksossa esitä numeerisia tuloksia, mutta niiden tutkimisella voi jatkaa
analyysiä.

Tavoite on tutkia seitsemän kysymyksen vastausvaihtoehtojen yhteyksiä,
miten ne asettuvat kaksiulotteiselle kartalle. Aikaisemmissa luvuissa etsittiin
...
Kuvasta 7.4 nähdään, että inertian selitysosuudet ovat paljon pienempiä, ja ratkaisu
...
Kuvassa 7.5 jokainen havainto on sarakevektoreiden keskiarvopiste. Sarakevektoreita ei voi
...
näkyvät paremmin. Kuvaan 7.6 on piirretty havaintopisteet, joista voi hahmottaa
...
ulottuvuuksiin ("dimensions of middleness").
